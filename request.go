...
func NewRequest(event json.RawMessage) (*http.Request, string, error) {
	var r struct {
		Version string `json:"version"`
	}
	if PayloadVersion == "" {
	...
	var apiType string
	if r.Version != "" {
		apiType = "HTTP" // HTTP API (v1.0/v2.0) has version field
	} else {
		apiType = "REST" // REST API has no version field
	}
